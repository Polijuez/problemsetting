...
// submission así se ve igual que `solo-chicos.cpp` -- mismo formato, misma
// cantidad de líneas -- y la única diferencia está en lo que el objeto *es*.
//
// En submissions.yml se declara `verdict: WA` y `score: 0`, y también es el
// sujeto de la comprobación 2 de `verify` (un envío que debe fallar, falla).  El
// fallo es en **todos** los lotes, así que este envío no aporta nada a ningún
// lote: el primer caso de cada subtarea ya lo rechaza y el resto queda en `SC`.
//
// Si el checker estuviera roto aceptando cualquier cosa, este envío daría AC y
// `verify` fallaría en la comprobación 2.  O sea: este archivo es evidencia de
...
int main() {
    ...
    // Un faro en la posición 1: ilumina 1, 2, 3 y las dos últimas posiciones.
    // Con n <= 5 daría justo en el óptimo; el generador nunca produce esos
    // tamaños (ver TAMANOS_ST1 en generator.py: el más chico es 6).
    std::cout << "1\n1\n";
    return 0;
}
